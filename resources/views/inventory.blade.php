...
.topbar-search input{width:100%;padding:8px 12px 8px 34px;border:1px solid var(--border);border-radius:10px;font-family:'Barlow',sans-serif;font-size:13px;color:var(--text);background:var(--bg);outline:none;transition:border-color .2s,box-shadow .2s,background .3s}
...
.tbl th{text-align:left;font-family:'Barlow Condensed',sans-serif;font-size:10px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);padding:11px 14px;border-bottom:1px solid var(--border);background:var(--surface2);white-space:nowrap;cursor:pointer;user-select:none;transition:background .3s,color .2s}
.tbl th:hover{color:var(--cyan)}
.tbl th .sort-icon{margin-left:4px;font-size:9px;opacity:.5}
.tbl th.sorted .sort-icon{opacity:1;color:var(--cyan)}
...
<div id="app">
  ...
  <div class="sidebar" id="sidebar">
    ...
    <div class="sidebar-footer">
      ...
      <button class="sidebar-footer-btn danger" onclick="openModal('modalLogout')"><i class="fa-solid fa-arrow-right-from-bracket"></i><span>Log Out</span></button>
    </div>
  </div>
...
<div class="modal-backdrop" id="modalItem">
  <div class="modal modal-lg">
    <div class="modal-header">
      <div class="modal-title" id="modalItemTitle">Add <span>Item</span></div>
    ...
    <div class="modal-body">
      ...
      <div class="form-row full">
        <div class="form-ctrl">
          ...
          <input type="file" id="iPhoto" accept="image/jpeg,image/png,image/webp" style="padding:6px;">
    ...
    <div class="modal-footer">
      <button class="btn btn-outline" onclick="closeModal('modalItem')">Cancel</button>
      <button class="btn btn-primary" onclick="saveItem()"><i class="fa-solid fa-save"></i> Save Item</button>
    </div>
  </div>
</div>
...
<div class="modal-backdrop" id="modalLogout">
  <div class="modal modal-sm">
    <div class="modal-header">
      <div class="modal-title">Log <span>Out</span></div>
      <button class="modal-close" onclick="closeModal('modalLogout')">&#x2715;</button>
    </div>
    <div class="modal-body" style="text-align:center;padding:24px;">
      <i class="fa-solid fa-arrow-right-from-bracket" style="font-size:36px;color:var(--danger);margin-bottom:14px;display:block;"></i>
      <p style="font-size:14px;color:var(--text);">Are you sure you want to log out?</p>
    </div>
    <div class="modal-footer" style="justify-content:center;">
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if (theme === 'dark') { toggle.classList.add('on'); knob.innerHTML = '<i class="fa-solid fa-sun"></i>'; }
  ...
  // Auto-fill barcode from SKU
  document.getElementById('iSku').addEventListener('input', function() {
    const bc = document.getElementById('iBarcode');
    if (!bc.dataset.manuallySet) bc.value = this.value;
  });
  document.getElementById('iBarcode').addEventListener('input', function() {
...
async function loadAll() {
  try {
    ...
    const [catData, supData, prodData] = await Promise.all([catRes.json(), supRes.json(), prodRes.json()]);
    ...
    document.getElementById('inventoryTbody').innerHTML =
      '<tr><td colspan="12" class="empty-state"><i class="fa-solid fa-circle-exclamation" style="color:var(--danger);font-size:28px;opacity:.6;display:block;margin-bottom:10px;"></i>Failed to load data.</td></tr>';
  }
}
...
function onCategoryChange() {
  const catId   = parseInt(document.getElementById('iCategory').value);
  const cat     = CATEGORIES.find(c => c.category_id === catId);
  const subSel  = document.getElementById('iSubcategory');
...
function onFilterCategoryChange() {
  const catId   = parseInt(document.getElementById('filterCategory').value);
  const cat     = CATEGORIES.find(c => c.category_id === catId);
  const subcats = cat?.subcategories || [];
...
function applyFilters() {
  ...
  const supId   = parseInt(document.getElementById('filterSupplier').value) || 0;
...
function sortBy(key) {
  ...
  if (th) {
    ...
    const icon = th.querySelector('.sort-icon');
    if (icon) icon.className = `fa-solid fa-sort-${SORT_DIR === 'asc' ? 'up' : 'down'} sort-icon`;
  }
  applyFilters();
}
...
function renderTable() {
  ...
  if (!total) {
    tbody.innerHTML = `<tr><td colspan="12"><div class="empty-state"><i class="fa-solid fa-box-open"></i><p>No items found matching your filters.</p></div></td></tr>`;
  ...
  tbody.innerHTML = visible.map(p => {
    const statusCls = p.status === 'out_of_stock' ? 'badge-red' : p.status === 'low_stock' ? 'badge-warn' : 'badge-green';
    ...
    return `<tr>
      ...
      <td>
        <div style="font-weight:700;font-size:13px;color:var(--text);line-height:1.3;">${p.name}</div>
        <div style="font-size:11px;color:var(--muted);margin-top:2px;">${p.brand}</div>
      </td>
      <td><span style="font-size:12px;color:var(--text2);">${p.category}</span></td>
      <td>${subcatHtml}</td>
      <td>${supplierHtml}</td>
      <td>${swatchHtml}</td>
      <td><span style="font-family:'Barlow Condensed',sans-serif;font-size:17px;font-weight:800;color:${qtyColor};">${p.stock}</span></td>
      <td><span style="font-family:'Barlow Condensed',sans-serif;font-size:14px;font-weight:700;color:var(--text);">₱${p.price.toLocaleString()}</span></td>
      <td><span style="font-size:12px;color:var(--muted);">${p.reorder}</span></td>
  ...
  renderPagination(totalPages, start, start + visible.length, total);
}

function renderPagination(totalPages, start, end, total) {
  const bar   = document.getElementById('paginationWrap');
  const delta = 2;
  const rs    = Math.max(1, PAGE - delta);
  const re    = Math.min(totalPages, PAGE + delta);

  let btns = '';
  if (rs > 1) { btns += `<button class="pg-btn" onclick="goPage(1)">1</button>`; if (rs > 2) btns += `<span class="pg-ellipsis">…</span>`; }
  for (let i = rs; i <= re; i++) btns += `<button class="pg-btn ${i===PAGE?'active':''}" onclick="goPage(${i})">${i}</button>`;
  if (re < totalPages) { if (re < totalPages-1) btns += `<span class="pg-ellipsis">…</span>`; btns += `<button class="pg-btn" onclick="goPage(${totalPages})">${totalPages}</button>`; }
  ...
  bar.innerHTML = `
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
      ...
      ${btns}
      <button class="pg-btn" onclick="goPage(${PAGE+1})" ${PAGE===totalPages?'disabled':''}><i class="fa-solid fa-chevron-right" style="font-size:9px;"></i></button>
      <span style="font-size:12px;color:var(--muted);margin-left:6px;">Show:</span>
      <select onchange="PAGE_SIZE=parseInt(this.value);applyFilters()" style="padding:4px 8px;border-radius:6px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:12px;cursor:pointer;">${sizes}</select>
    </div>`;
}
...
function openAddItem() {
  ...
  document.getElementById('iSubcategory').innerHTML = '<option value="">None</option>';
...
function openEditItem(id) {
  ...
  if (!p) return;
  editingId = id;
  document.getElementById('modalItemTitle').innerHTML = 'Edit <span>Item</span>';
...
async function saveItem() {
  ...
  const category_id  = parseInt(document.getElementById('iCategory').value);
  ...
  try {
    ...
    if (data.status === 'success') {
      ...
      const msg = data.errors ? Object.values(data.errors).flat().join(', ') : (data.message || 'Error saving item.');
...
function showToast(msg, type='success') {
  ...
  t.id = 'rfmoto-toast';
  t.style.cssText = `position:fixed;bottom:28px;right:28px;z-index:99999;background:${colors[type]||colors.cyan};color:#fff;padding:12px 20px;border-radius:10px;font-size:13px;font-weight:600;box-shadow:0 4px 24px rgba(0,0,0,.25);animation:fadeInUp .3s ease;`;
  t.textContent = msg;
